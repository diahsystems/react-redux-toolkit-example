...
import { renderWithRedux, rootInitialState } from 'utilities/test-helpers'
import axios from 'axios'
import { incrementByAmountAsync } from './counterSlice'
...
describe('<Counter />', () => {
  ...
  //   renderWithRedux(<Counter />, {
  //     ...rootInitialState,
  //     counter: { ...rootInitialState.counter, loading: 'pending' },
  //   })
  ...
  //   expect(mockStore.getActions()[0].type).toEqual(incrementByAmountAsync.pending.type)
  //   expect(mockStore.getActions()[1].type).toEqual(incrementByAmountAsync.fulfilled.type)
  ...
  //   await waitFor(() => null, { timeout: 500 })
  //   expect(mockStore.getActions()[0].type).toEqual(incrementByAmountAsync.pending.type)
  //   expect(mockStore.getActions()[1].type).toEqual(incrementByAmountAsync.rejected.type)
...
describe('CounterSlice', () => {
  test('sets loading on fetch start', () => {
    expect(
      counterReducer(
        { ...rootInitialState.counter, loading: 'pending' },
        incrementByAmountAsync.pending
      )
  ...
  test('sets value and stop loading on fetch success', () => {
    expect(
      counterReducer(
        { ...rootInitialState.counter, loading: 'idle' },
        { type: incrementByAmountAsync.fulfilled.type, payload: 100 }
      )
  ...
  test('sets error and stop loading on fetch error', () => {
    expect(
      counterReducer(
        { ...rootInitialState.counter, loading: 'idle' },
        { type: incrementByAmountAsync.rejected, payload: 'Some error message.' }
      )
    ).toEqual({
  ...
  test('increases number by one', () => {
    expect(counterReducer({ ...rootInitialState.counter, value: 1 }, increment)).toEqual({
      ...rootInitialState.counter,
      value: 2,
    })
  })
  ...
  test('decreases number by one', () => {
    expect(counterReducer({ ...rootInitialState.counter, value: 1 }, decrement)).toEqual({
      ...rootInitialState.counter,
      value: 0,
    })
  })
